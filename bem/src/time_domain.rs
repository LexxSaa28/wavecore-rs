use crate::{BemError, BemResult, BemProblem, BemResults};
use wavecore_meshes::Mesh;
use wavecore_green_functions::GreenFunction;
use wavecore_matrices::{Matrix, Vector};
...
impl TimeDomainSolver {
    ...
    pub fn solve_time_domain(&self, problem: &TimeDomainProblem) -> BemResult<TimeDomainResults> {
        ...
        for step in 0..self.time_params.num_steps {
            ...
            match self.time_params.integration_scheme {
                ...
                _ => {
                    return Err(BemError::SolverError("Unsupported integration scheme".to_string()));
                }
            }
    ...
    pub fn calculate_impulse_responses(&self, frequencies: &[f64]) -> BemResult<ImpulseResponseData> {
        if frequencies.is_empty() {
            return Err(BemError::InvalidInput("Empty frequency array".to_string()));
        }
        ...
        // Compute added mass at infinite frequency (simplified)
        let added_mass_inf = Matrix::from_diagonal(&nalgebra::DVector::from_vec(vec![0.1; num_dofs]));
        let damping = Matrix::from_diagonal(&nalgebra::DVector::from_vec(vec![0.05; num_dofs]));
    ...
    pub fn apply_memory_effects(&self, history: &[f64], dt: f64) -> BemResult<f64> {
        if history.is_empty() {
            return Ok(0.0);
        }
        ...
        for i in 0..kernel_length {
            let kernel_value = (-i as f64 * dt / 0.5).exp(); // Exponential decay kernel
    ...
    fn compute_impulse_response_ij(&self, i: usize, j: usize, frequencies: &[f64], time_vector: &[f64]) -> BemResult<Vec<f64>> {
    ...
    fn initialize_results(&self, problem: &TimeDomainProblem) -> BemResult<TimeDomainResults> {
    ...
    fn compute_wave_elevation(&self, time: f64, wave_conditions: &WaveConditions) -> BemResult<f64> {
    ...
    fn interpolate_time_series(&self, t: f64, time_series: &TimeSeries) -> BemResult<f64> {
    ...
    fn compute_hydrodynamic_forces(&self, time: f64, positions: &[f64], velocities: &[f64], 
                                  accelerations: &[f64], problem: &TimeDomainProblem) -> BemResult<Vec<f64>> {
        ...
        for i in 0..num_dofs {
            for j in 0..num_dofs {
                forces[i] -= problem.body_properties.mass[(i, j)] * accelerations[j];
            }
        }
        ...
        for i in 0..num_dofs {
            for j in 0..num_dofs {
                forces[i] -= problem.body_properties.hydrostatic[(i, j)] * positions[j];
            }
        }
        ...
        for i in 0..num_dofs {
            for j in 0..num_dofs {
                forces[i] -= problem.body_properties.linear_damping[(i, j)] * velocities[j];
            }
        }
    ...
    /// Compute external forces
    fn compute_external_forces(&self, time: f64, external_forces: &ExternalForces) -> BemResult<Vec<f64>> {
    ...
    fn forward_euler_step(&self, dt: f64, positions: &mut [f64], velocities: &mut [f64], 
                         accelerations: &mut [f64], forces: &[f64], problem: &TimeDomainProblem) -> BemResult<()> {
        // Update accelerations: M * a = F
        for i in 0..accelerations.len() {
            accelerations[i] = forces[i] / problem.body_properties.mass[(i, i)].max(1e-10);
        }
    ...
    fn store_step_results(&self, results: &mut TimeDomainResults, step: usize, time: f64,
                         ...
                         forces: &[f64], wave_elevation: f64) -> BemResult<()> {
    ...
    /// Finalize results
    fn finalize_results(&self, mut results: TimeDomainResults) -> BemResult<TimeDomainResults> {
...
impl MemoryEffects {
    ...
    pub fn compute_memory_forces(&self, time: f64, velocities: &[f64]) -> BemResult<Vec<f64>> {
    ...
    fn convolve_history(&self, history: &[f64], kernel: &[f64]) -> BemResult<f64> {
        let mut result = 0.0;
...
impl ImpulseResponseData {
    ...
    pub fn empty() -> Self {
        Self {
            ...
            added_mass_inf: Matrix::zeros(6, 6),
            damping: Matrix::zeros(6, 6),
